//////////////////////////////////////////////////////////////////////
//	created:	2014/08/19
//	filename: 	src\GSLib\sceneSystem\GM\originalSceneMgr.cpp
//	author:		mazhejiang
//	
//	purpose:	
//////////////////////////////////////////////////////////////////////
...
bool COriginalSceneMgr::_loadSceneConfig(const std::string& a_configPath)
{
	...
	if (!tableSheet.loadXmlFile(scenePath)) {
		BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "加载场景配置失败[%s]", scenePath.c_str());
		return false;
	}
	...
	try {
		...
		for (BSLib::uint32 i=0; i < table.getRowCount(); ++i) {
			...
			if (m_originalSceneHashMap.find(sceneTPID) != m_originalSceneHashMap.end()) {
				BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "SceneTPID重复[%d]", sceneTPID);
				return false;
			}
		...
		BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "加载场景文件失败[%s]", scenePath.c_str());
		return false;
	}
	return true;
}
	...
	for (; it != m_originalSceneHashMap.end(); ++it) {
		...
		if (BSLib::Utility::CFileInfo::isExist(fsmFile)) {
			if (!CSceneGMFSMMgr::singleton().loadFSMFile(fsmFile)) {
				BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "加载场景文件失败[%s]", fsmFile.c_str());
				return false;
			}
		}
		...
		if (BSLib::Utility::CFileInfo::isExist(btFile)) {
			if (!CSceneGMBTMgr::singleton().loadBTFromFile(btFile)) {
				BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "加载场景文件失败[%s]", btFile.c_str());
				return false;
			}
		}
	}
...
bool COriginalSceneMgr::_initOriginalScene(const std::string& a_configPath)
{
	...
	for (; it != m_originalSceneHashMap.end(); ++it) {
		...
		//	BSLIB_LOG_INFOR(ETT_GSLIB_SCENESYSTEM, "加载场景配置失败[%s]", a_configPath.c_str());
		//	return false;
		//}
		//if (!scene->init()) {
		//	return false;
