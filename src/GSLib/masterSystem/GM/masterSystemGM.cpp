...
bool CMasterSystemGM::_loadGameConfig(const std::string& a_configPath)
{
	return GSLib::DBSystem::CDBCommonSystem::_loadGameConfig(a_configPath);
}

bool CMasterSystemGM::_initServerMsg(BSLib::Framework::CMsgExecMgr* a_msgExecMgr)
{	
...
void CMasterSystemGM::_cbServerEnter(const GFLib::SServerID& a_serverID, const std::string& a_key)
{	
...
void CMasterSystemGM::_cbServerLeave(const GFLib::SServerID& a_serverID, const std::string& a_key)
{	
	if(a_serverID.m_type == GSLib::SRVTYPE_CENTERSERVER)
	{
		m_cnKey = "";
